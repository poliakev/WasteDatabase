...
for driver in drivelist:
    ...
    c.execute(statement, [driver[0], 'driver', (driver[1][-3:] + str(marker)), sql.Binary(hashed_password)])
...
for man in manlist:
    ...
    hashed_password = pbkdf2_hmac(hash_name, bytearray(password, 'ascii'), bytearray(salt, 'ascii'), iterations)
    statement = '''INSERT INTO users (user_id, role, login, password) values(?,?,?,?);''' #not sure if semicolon is necessary
    c.execute(statement, [man[0], 'account manager', (man[1][-3:] + str(marker)), sql.Binary(hashed_password)])
...
for sup in suplist:
    ...
    hashed_password = pbkdf2_hmac(hash_name, bytearray(password, 'ascii'), bytearray(salt, 'ascii'), iterations)
    statement = '''INSERT INTO users (user_id, role, login, password) values(?,?,?,?);''' #not sure if semicolon is necessary
    c.execute(statement, [sup[0], 'supervisor', (sup[1][-3:] + str(marker)), sql.Binary(hashed_password)])
...
for dis in dislist:
    ...
    hashed_password = pbkdf2_hmac(hash_name, bytearray(password, 'ascii'), bytearray(salt, 'ascii'), iterations)
    statement = '''INSERT INTO users (user_id, role, login, password) values(?,?,?,?);''' #not sure if semicolon is necessary
    c.execute(statement, [dis[0], 'dispatcher', (dis[1][-3:] + str(marker)), sql.Binary(hashed_password)])
